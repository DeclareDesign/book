...
bind_cols(assignment_1, assignment_2, assignment_3) |> 
  ...
  head(5) |> 
  kable(booktabs = TRUE, align = "c", caption = "Three random assignments from the same random assignment step.", digits = 3) |> kable_styling()
```

Every step in a research design can be declared using one of the `declare_*` functions. Table @tbl-declarationfunctions collects these according to the four elements of a research design. In @sec-ch13, we detail how to build each kind of step.
