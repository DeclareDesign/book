...
Here, the outcome $Y$ is related to $Z$ and $U$ by some function $f_Y$, but the details of what the function $f_Y$ is---whether $Z$ has a positive or negative effect on $Y$, for example---are left unstated in this nonparametric model. The DAG in Figure @fig-ch6fig1 encodes this model in graphical form. We use a blue circle around the treatment assignment to indicate that $Z$ is randomly assigned as part of the data strategy.

![Directed acyclic graph of a randomized experiment](/figures/figure-6-1){#fig-ch6fig1}
...
![A directed acyclic graph with an explanatory variable of interest (D), an outcome of interest (Y), a mediator (M), a confounder (X1), a moderator (X2), an instrument (Z), and a collider (K).](/figures/figure-6-2){#fig-ch6fig2}
...
It is also possible to use the distribution over quantities in a model to represent our *uncertainty* over some quantities and in this way let our diagnostics integrate over our uncertainty. We did this already in Declaration @def-declarationch2dec1 where we let the treatment effect be a random draw from a stipulated distribution. In doing this the success function represents expected success with respect to this distribution over treatment effects.   
