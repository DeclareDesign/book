...
In Figure \@ref(fig:figure-12-1), we illustrate the results of our redesign exercise across all four diagnosands. The number of citizens per village is plotted on the horizontal axis and the value of the diagnosand is shown on the vertical axis. The plot is faceted by diagnosand and each line represents a different possible number of villages. We focus here on the social treatment only.

```{r figure-12-1, fig.cap = "Redesign of Gulzar and Khan (2020)", echo = FALSE}
imgsufx <- if(is_html_output()) "svg" else "pdf"
include_graphics(path = paste0("figures/figure_12.1.", imgsufx))
```
