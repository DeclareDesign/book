...
run_design(declaration_9.2) |> 
  ...
  kable(booktabs = TRUE, align = "c", digits = 2, caption = "Estimates from the test of the mean age equalling 20 from Italian villages example") |> kable_styling()
```
...
get_rdddr_file_local("diagnosis_9.5") |>
  knitr::kable(digits = 3, caption = "Bootstrapped standard error estimate") |> kable_styling()
```
...
get_estimates(design = declaration_9.5, data = clingingsmith_etal) |> 
  ...
  kable(digits = 3, caption = "Analytic standard error estimate") |> kable_styling()
```
