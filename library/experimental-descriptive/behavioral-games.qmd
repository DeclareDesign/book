...
We illustrate using the "trust" game in which we specify three common inquiries and use a standard design in Declaration \@ref(def:declaration-17-6). The design is successful at generating unbiased estimates of the first inquiry but runs into problems with the other two.   
...
declaration_17.6 |>
  ...
  kable(
    booktabs = TRUE,
    align = "c",
    caption = "Trust game sample data",
    digits = 2
  )
```
...
Diagnosis \@ref(lem:diagnosis-17-5) illustrates the properties of the trust game design.

:::{.lemma #diagnosis-17-5}
...
```{r behavioraldiagnosis, echo = FALSE, fig.cap="Diagnosis of bias in the analysis of trust games with and without deception"}
imgsufx <- if(is_html_output()) "svg" else "pdf"
include_graphics(path = paste0("figures/figure_17.5.", imgsufx))
```
