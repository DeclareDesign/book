...
When we cannot credibly assert that we have controlled for all confounding variables as in a selection-on-observables design (see Section @sec-ch16s2) and when we do not think the parallel trends assumption is likely to hold in a difference-in-differences design (see @sec-ch16s3), we might have to give up on our goal of drawing causal inferences. 
