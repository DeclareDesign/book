...
```{r statdag, fig.cap = "Directed acyclic graph of a two-arm randomized experiment", echo=FALSE, fig.height = 3.5, fig.width = 7}
imgsufx <- if(is_html_output()) "svg" else "pdf"
include_graphics(path = paste0("figures/figure_18.1.", imgsufx))
```

:::{.definition #declaration-18-1} 
...
When treatments are randomized, whether we adjust for pre-treatment covariates makes little difference for bias. By contrast, when treatments are not randomized, we often do need on adjust for covariates in order to account for the confounding introduced by "omitted variables" (see \@ref(selection-on-observables)). 
...
```{r figure-18-3, echo=FALSE, fig.cap="Comparing the Lin estimator to OLS and difference-in-means, varying fraction assigned to treatment and correlation of potential outcomes with the covariate.", fig.height=6.5, fig.width=6.5}
imgsufx <- if(is_html_output()) "svg" else "pdf"
include_graphics(path = paste0("figures/figure_18.3.", imgsufx))
```
