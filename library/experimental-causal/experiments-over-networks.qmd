...
First, we load the Fairfax County, Virginia, voting precincts shapefile, removing one singleton voting precinct, and we plot the precincts in Figure \@ref(fig:figure-18-15).

```{r figure-18-15, fig.cap="Voting Precincts in Fairfax County, Virginia", fig.height=6.5, fig.width=6.5, echo=FALSE}
imgsufx <- if(is_html_output()) "svg" else "pdf"
include_graphics(path = paste0("figures/figure_18.15.", imgsufx))
```
...
The maps in Figure \@ref(fig:figure-18-16) show how this procedure generates differential probabilities of assignment to each exposure condition. Units that are in denser areas of the county are more likely to be in the `Indirect Exposure Only` and `Direct and Indirect Exposure` conditions than those in less dense areas.

```{r figure-18-16, echo=FALSE, fig.cap="Probabilities of assignment to each of four conditions depend on position in geographic space.", fig.height = 6.5, fig.width=6.5}
imgsufx <- if(is_html_output()) "svg" else "pdf"
include_graphics(path = paste0("figures/figure_18.16.", imgsufx))
```
...
```{r figure-18-17, fig.cap='Sampling distribution of  the Hajek and Horvitz-Thompson estimators of direct, indirect, and direct plus indirect effects. The vertical lines refer to the true values of the inquiries.', echo = FALSE, fig.width = 6.5, fig.height = 6.5}
imgsufx <- if(is_html_output()) "svg" else "pdf"
include_graphics(path = paste0("figures/figure_18.17.", imgsufx))
```
