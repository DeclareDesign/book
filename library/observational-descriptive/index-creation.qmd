...
In Figure \@ref(fig:figure-15-6) we show that the correlation between all the indices and the underlying quantity is quite good, even though the strength of the correlations for some of the components is weak. Trickier however is being sure we have an interpretable  *scale*.

```{r figure-15-6, echo = FALSE, fig.cap = "Correlations between the latent variable, measured components, and indices."}
imgsufx <- if(is_html_output()) "svg" else "pdf"
include_graphics(path = paste0("figures/figure_15.6.", imgsufx))
```
...
```{r figure-15-7, fig.cap = "Distribution of estimates from different approaches to generating indices", fig.width = 6.5, fig.height = 2.5, echo = FALSE}
imgsufx <- if(is_html_output()) "svg" else "pdf"
include_graphics(path = paste0("figures/figure_15.7.", imgsufx))
```
