...
In Declaration \@ref(def:declaration-15-6), our inquiry is the average level of the latent variable among units whose binary covariate `X` is equal to one.
...
In Figure @fig-figure-15-6 we show that the correlation between all the indices and the underlying quantity is quite good, even though the strength of the correlations for some of the components is weak. Trickier however is being sure we have an interpretable  *scale*.

![Correlations between the latent variable, measured components, and indices.](/figures/figure-15-6){#fig:figure-15-6}

:::{.lemma #diagnosis-15-5}
...
Figure @fig-figure-15-7 shows the distribution of estimates from different approaches to generating indices. A few features stand out from the distribution of estimates. 
...
![Distribution of estimates from different approaches to generating indices](/figures/figure-15-7){#fig:figure-15-7}
