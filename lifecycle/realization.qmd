...
In @sec-ch21, we declared the design for @bonilla_tillery_2020 following their preanalysis plan. In doing so, we declared an answer strategy in code. In our populated PAP (shown in @fig-populatedpapcontactsheet and available in the appendix), we can run that same answer strategy code, but swap out the simulated data for the real data collected during the study. 
...
We provide illustrations for both of these in @tb1-reconciliation1 and @tb1-reconciliation2.
